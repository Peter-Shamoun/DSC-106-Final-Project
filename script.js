...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  try {
    ...
    setupFiltersLinePlot();

    // Create initial visualization
    createScatterPlot(data, "all");
    createLinePlot(data, "all");
  } catch (error) {
...
function setupFiltersLinePlot() {
  ...
  const sexDropdownLine = d3.select("#filter-container2")
    ...
    .on("change", function () {
      const selectedSex = this.value;
      createLinePlot(globalData, selectedSex);
    });
...
// Create scatter plot visualization
function createLinePlot(data, filteredSex = "all") {
  // Filter data based on sex selection
